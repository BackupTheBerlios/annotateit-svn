...
unless ($authInfo->{LoggedIn}) {
  my $vars = {scriptdir => $scriptdir,
	      ...
	      formAction => "searchAnnotations.cgi"};
  print $C->header;
...
my $user = User->load( dbh => $dbh,
		       ID => $authInfo->{UserID} );
unless ($user->hasPrivilege("Other.SearchAnnotations") or
...
unless ($action eq "Search") {
  ...
  $template->process("SearchAnnotationsForm.html",$vars) or die $template->error;
  exit;
}
my $search = AnnotationsSearch->new( CurrentUser => $user, 
				     dbh => $dbh,
...
print $C->header;
$template->process("SearchAnnotationsResults.html",$vars) or die $template->error;
exit;
